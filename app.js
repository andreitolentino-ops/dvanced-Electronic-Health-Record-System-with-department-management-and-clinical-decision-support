...
function applyRoleUI(){
  ...
  // Hide Treatment Plan tab from nurses - only doctors can access it
  ...
  if(currentRole === 'nurse' || currentRole === 'guest'){
  ...
  // Hide Doctor Notes tab from nurses and guests
  ...
  // disable doctor save button for nurses
  const doctorSave = document.querySelector('[data-save="doctor"]');
  if(doctorSave) doctorSave.disabled = (currentRole === 'nurse' || currentRole === 'guest');
  
  // disable assessment save button for nurses
  const assessmentSave = document.querySelector('[data-save="assessment"]');
  if(assessmentSave) assessmentSave.disabled = (currentRole === 'nurse' || currentRole === 'guest');
  
  // med tab controls: allow nurses to add/edit meds but disable some features for guests
  const addMedBtn = document.getElementById('addMedRow');
  const normalizeBtn = document.getElementById('normalizeIVs');
  if(addMedBtn) addMedBtn.disabled = (currentRole === 'guest');
  if(normalizeBtn) normalizeBtn.disabled = (currentRole === 'guest');
}
